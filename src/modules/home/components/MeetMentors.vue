	...
	<div class="meetmentors" :aria-label="ariaLabel" role="region">
		...
		<div class="meetmentorsWrapper">
			<ul class="slideAccordion">
				<li
					v-for="mentor in this.data.innerText.Mentores"
					:key="mentor.id"
					class="slideAccordion__items"
					:class="mentor.id === 0 ? 'active' : ''"
					tabIndex="0"
				>
					...
					<div class="slideAccordion__items--image">
						<img
							:src="
								require(`@/assets/images/mentors/${mentor.image}`)
							"
							alt=""
						/>
					</div>
					<div class="slideAccordion__items--content">
						...
						<p class="country">
							<span class="icon">
								<img
									:src="
										require(`@/assets/images/country/${mentor.flag}`)
									"
									alt=""
								/>
							</span>
						...
						<a
							href="#"
							@click.prevent="openModal(mentor)"
							class="btn btn-yellow"
							:aria-describedby="'edition' + mentor.id"
							:role="'edition' + mentor.id"
							>{{ $t('buttons.explore') }}</a
						>
					</div>
				</li>
...
export default {
	...
	mounted() {
		var items = document.querySelectorAll(
			'.slideAccordion__items:not(.active)'
		);
		var self = this;
		items.forEach((item) => {
	...
	methods: {
		...
		removeActive() {
			var itemsInner = document.querySelectorAll(
				'.slideAccordion__items'
			);
